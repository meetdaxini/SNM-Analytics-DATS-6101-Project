...
```
```{r}
# Replace missing values in integer columns with mean and numeric columns with median
data_filled <- renamed_data %>%
  mutate_if(is.integer, ~ifelse(is.na(.), mean(., na.rm = TRUE), .)) %>%
  mutate_if(is.numeric, ~ifelse(is.na(.), median(., na.rm = TRUE), .))
```

```{r}
# Analyze missing values in the dataset
missing_values_check <- data_filled %>%
