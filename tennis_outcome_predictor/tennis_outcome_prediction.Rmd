...
```{r}
nodatastat <- kable(non_null_stats, digits = 2)
```
The table shows that there are no significant data available from 1968 to 1990
`r nodatastat`
...
```{r}
# Renaming the final variable
final_data = renamed_data
```

```{r}
# Number of columns of Final data
col_final <- ncol(final_data)
```

There are `r col_final` number of columns in the dataset
...
```{r}
# Correlation map of the numeric variables
...
```{r}
# Understand the unique values of player 0 hand
...
```{r}
#Setting the seed & split the dataset into Train and Test
set.seed(123)
...
```{r}
# Prediction using Test dataset & confusion matrix
...
```{r}
# Split data into training and testing sets and SVM Model
...
```{r}
# Confusion Matrix and Accuracy, Precision, Recall & F1-score
