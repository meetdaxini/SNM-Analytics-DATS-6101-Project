...
# Plot & numeric representation of year-wise match statistics
...
The Jeff Sackman Dataset contains multiple sets of files. We use the Sackman_tennis_atp-master project which contains match level statistics from the year 1968. Since more extensive match level statistics are only available from the year 1991, we concentrate on the ATP match level files from 1991 to 2023.
...
```
The table shows the count and percentages of the missing values:
*missing_values*
...
```
```{r}
# Predict the winner using the SVM model on the testing set
svm_pred 
# Evaluate the accuracy of the SVM model
#accuracy <- sum(svm_pred == test_set$winner)/nrow(test_set)
